...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        */
        ...
        this.db.collection("cart").find({userId: userId}).toArray((err, doc) => {
    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         */
         this.db.collection("cart").aggregate([
           ...
           if(doc.length === 0) {callback(null);}
           else {callback(doc[0].items);}
         })
    ...
    this.updateQuantity = function(userId, itemId, quantity, callback) {
        ...
        */
        ...
        if(quantity == 0) {
            ...
            this.db.collection("cart").update(
              {userId},
        ...
        this.db.collection("cart").find({userId}).next((err, userCart) => {
          callback(userCart);
        });
