...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        this.db.collection("item").aggregate(pipeline).toArray( (err, doc) => {
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
         var query = (category === "All") ? {} : {"category": category};
         //var query = {};
           this.db.collection("item").find(query).toArray((err, doc) => {
             numItems = doc.length;
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        this.db.collection("item").find({$text: {$search: query}}).toArray((err, doc) => {
          numItems = doc.length;
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection("item").find({_id: itemId}).toArray((err, item) => {
          callback(item[0]);
        });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        this.db.collection("item").update({_id: itemId}, {$push: {reviews: reviewDoc}}).then((err, doc) => {
          callback(doc);
        });
